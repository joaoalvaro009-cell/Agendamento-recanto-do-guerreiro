...
export const Route = createFileRoute("/meu-agendamento")({
  ...
  head: () => ({
    meta: [
      { title: "Meu agendamento — Recanto do Guerreiro" },
...
function MeuAgendamentoPage() {
  ...
  async function cancel() {
    ...
    setSubmitting(true);
    const { data, error } = await supabase.rpc("cancel_appointment_by_code", {
            ...
            <div className="mt-7 rounded-xl border border-border/60 bg-background/40 p-5">
                          ...
                          return (
                            <button
                              ...
                              className={`rounded-lg border p-2.5 text-xs ${
                                selected ? "border-gold bg-gold/10 text-gold" : "border-border/60"
                              }`}
                            >
                              {formatDatePretty(d)}
                            </button>
              ...
              {appt.status === "cancelled" && (
                <div className="mt-5 text-center">
                  <Link
                    to="/agendar"
                    className="inline-flex rounded-full bg-gradient-gold px-6 py-2.5 text-sm font-semibold text-primary-foreground shadow-gold"
                  >
