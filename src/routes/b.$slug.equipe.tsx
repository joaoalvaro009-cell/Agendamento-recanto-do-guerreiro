...
export const Route = createFileRoute("/b/$slug/equipe")({
  head: () => ({
    meta: [
      { title: "Nossa equipe" },
      { name: "description", content: "Conheça os barbeiros que atendem você." },
    ],
  }),
  ...
  return (
    <section className="mx-auto max-w-6xl px-4 py-16 sm:px-6 sm:py-24">
      <div className="mx-auto max-w-2xl text-center">
        ...
        <h1 className="mt-3 font-display text-4xl font-semibold tracking-tight sm:text-5xl">Profissionais à sua altura</h1>
        <p className="mt-4 text-muted-foreground">Mãos experientes, olho clínico e respeito total pelo cliente.</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="mx-auto mt-14 grid max-w-4xl gap-8 sm:grid-cols-2">
          {team.map((m) => {
            ...
            return (
              <article key={m.id} className="group overflow-hidden rounded-2xl border border-border/60 bg-surface/60 shadow-card transition hover:border-gold/60 hover:shadow-gold">
                <div className="relative aspect-[4/5] overflow-hidden">
                  <img src={img} alt={`Retrato profissional do barbeiro ${m.name}`} className="h-full w-full object-cover transition duration-700 group-hover:scale-105" loading="lazy" />
