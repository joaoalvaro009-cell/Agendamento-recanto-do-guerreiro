...
export const Route = createFileRoute("/b/$slug/")({
  head: () => ({
    meta: [
      { title: "Agende sua barbearia online" },
      { name: "description", content: "Cortes precisos, barba na navalha e ambiente premium. Agende online em segundos." },
...
function Home() {
  ...
  useEffect(() => {
    ...
    fetchTeam(tid).then(setTeam).catch(() => {});
    fetchPlans(tid).then(setPlans).catch(() => {});
    fetchServices(tid).then(setServices).catch(() => {});
    fetchTestimonials(tid).then(setTestimonials).catch(() => {});
  }, [t.tenant_id]);
  ...
  const renderHeroTitle = () => {
    if (!heroHighlight || !heroTitle.includes(heroHighlight)) return heroTitle;
    ...
    <>
        ...
        <div className="mx-auto flex min-h-[88vh] max-w-6xl flex-col justify-center px-4 py-20 sm:px-6">
          <div className="max-w-2xl">
            <p className="inline-flex items-center gap-2 rounded-full border border-gold/40 bg-gold/5 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.22em] text-gold">
              <Sparkles className="h-3 w-3" /> {heroBadge}{t.city && ` · ${t.city}`}
            </p>
            ...
            <div className="mt-10 flex flex-wrap gap-x-8 gap-y-3 text-sm text-muted-foreground">
              {t.hours_text && <div className="flex items-center gap-2"><Clock className="h-4 w-4 text-gold" /> {t.hours_text}</div>}
              {t.address && <div className="flex items-center gap-2"><MapPin className="h-4 w-4 text-gold" /> {t.address}</div>}
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-6xl px-4 py-20 sm:px-6">
        <div className="flex items-end justify-between gap-4">
          ...
          <Link to="/b/$slug/servicos" params={{ slug }} className="hidden text-sm text-gold hover:underline sm:inline">
            Ver todos →
          </Link>
        </div>
        ...
        <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {services.map((s) => (
            <div key={s.id} className="group rounded-2xl border border-border/60 bg-surface/60 p-6 shadow-card transition hover:border-gold/60 hover:shadow-gold">
              <div className="flex items-center justify-between">
                <Scissors className="h-5 w-5 text-gold" />
                <span className="font-display text-2xl font-semibold text-gold">R${s.price}</span>
              </div>
      ...
      <section className="mx-auto max-w-6xl px-4 py-20 sm:px-6">
        ...
        <div className="mt-12 grid gap-6 lg:grid-cols-3">
          {plans.map((p) => (
            <div key={p.id} className={`relative rounded-2xl border p-7 shadow-card transition ${
        ...
        <div className="mt-10 text-center">
          <Link to="/b/$slug/planos" params={{ slug }} className="inline-flex rounded-full border border-gold/60 px-7 py-3 text-sm font-semibold text-gold hover:bg-gold/10">
            Conhecer detalhes
          </Link>
        </div>
      </section>
